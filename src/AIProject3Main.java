 ...
 * Please use "d" to test Unit Test 3. I have included "f" which was my original Unit Test 3, but it takes much
 * longer to compile and requires much more memory.
...
public class AIProject3Main extends PApplet {
	...
	public void showInstructions() {
		...
		text("Press d for Project 3: Unit Test 3 (faster)", width/2, height*24/28);
		text("Press f for Project 3: Unit Test 3", width/2, height*26/28);
		fill(240, 75, 90);
		textSize(10);
		text("(now generated by the Order M Generator)", width/2, height*8/28);
		text("(RECOMMENDED: uses unique sequences for init sequences)", width/2, height*49/56);
		text("(original compiles slowly, uses probability generator for init sequences)", width/2, height*53/56);
	}
